...
let customSites = [];
let popularSites = { youtube: false, chatgpt: false, instagram: false };
...
function buildPacForProfile(p) {
  ...
  if (selectiveMode) {
    ...
    if (popularSites.instagram) conditions.push("dnsDomainIs(host, 'instagram.com') || dnsDomainIs(host, 'cdninstagram.com')");
  ...
  switch (msg.type) {
    ...
    case 'test':
      if (selectiveMode) {
        ...
        if (popularSites.instagram && (currentHost.includes('instagram.com') || currentHost.includes('cdninstagram.com'))) isSelected = true;
        if (!isSelected) {
